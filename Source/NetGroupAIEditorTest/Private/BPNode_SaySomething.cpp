...
#include "NetGroupAI/CalcSumBlueprintFunctionLibrary.h"
...
void UBPNode_SaySomething::GetNodeContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	...
	if (!Context->bIsDebugging)
	{
		static FName operatorName = FName("delete pin");
		FText operatorStr = FText::FromString( "operator node");
		...
		if (Context->Pin != NULL)
		{
			...
			if ((Context->Pin->Direction == EGPD_Input)&&(Context->Pin->ParentPin == nullptr))
			{
				FToolMenuSection& Section = Menu->AddSection(operatorName, operatorStr);
				Section.AddMenuEntry("RemovePin", 
									FText::FromString(TEXT("Remove Word")),
									FText::FromString(TEXT("Remove Word from input")), 
									FSlateIcon(),
									FUIAction(FExecuteAction::CreateUObject(const_cast<UBPNode_SaySomething*>(this),
...
void UBPNode_SaySomething::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	if (ExecPin && ThenPin) {
		...
		FName MyFunctionName = GET_FUNCTION_NAME_CHECKED(UCalcSumBlueprintFunctionLibrary, SaySomething_Internal);

		UK2Node_CallFunction* CallFuncNode = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
		CallFuncNode->FunctionReference.SetExternalMember(MyFunctionName, UCalcSumBlueprintFunctionLibrary::StaticClass());
